...
foundanswer = 0
while foundanswer == 0:
    guess[foundanswertime] = '1234'
    foundanswertime += 1
    if foundanswertime >= 1:
        for compareanswer in range(foundanswertime):
            compareresult = compare(guess[foundanswertime].zfill(4),guess[compareanswer].zfill(4))
            a[foundanswertime] = compareresult[0]
            b[foundanswertime] = compareresult[1]
            ok = 0
            if a[foundanswertime] == a[compareanswer]:
                if b[foundanswertime] == b[compareanswer]:
                    ok = 1
    compareresult = compare(guess[foundanswertime].zfill(4),guess[compareanswer].zfill(4))  
